...
const MainAdminUserList = ({ data,reCheck,setRecheck }) =>{
  ...
  const suspendUser=async()=>{
   ...
   const dataUser = await axiosReq("MainAdmin/SuspendAgent",{
    AgentId:id
   });
...
setShowBan(false)
   ...
   const dataUser = await axiosReq("MainAdmin/DeleteAgent",{
    AgentId:id
   });
   if (dataUser?.status == 200 || dataUser?.status == 204 || dataUser?.status == 201) {
setRecheck(!reCheck)
