...
const MainAdminRequestList = ({ data, reCheck, setRecheck }) => {
  ...
  const tableBody = data?.map((tableRow) =>
    <tr key={tableRow.id} className="border-b border-b-borderGray">
      ...
      <td style={{ fontFamily: 'shabnam' }} className="py-4 text-xs text-center px-4 ">  <div className="flex justify-center">
        <Link to={'/MainAdmin/requestView/' + tableRow?.requestId} className="mx-2" >
          <Eye />
        </Link>
      </div>
      </td>
      <td style={{ fontFamily: 'shabnam' }} className="py-4 text-xs text-center px-4 ">  <div className="flex justify-center">
        <Link to={'/MainAdmin/reportView/' + tableRow?.requestId} className="mx-2" >
          <Eye />
        </Link>
      </div>
      </td>
        ...
        <table class="table-auto w-full  ">
          <thead className="bg-darkGray h-11 rounded-t-2xl w-full whitespace-nowrap overflow-x-scroll" style={{ borderRadius: '20px' }}>
            <tr className="text-white  p-6 whitespace-nowrap overflow-x-scroll" style={{ borderRadius: '20px' }}>
              ...
              <th style={{ fontFamily: 'Shabnam' }} className="text-center   md:px-4 text-sm" >مشاهده درخواست</th>
              <th style={{ fontFamily: 'Shabnam' }} className="text-center   md:px-4 text-sm" > مشاهده گزارش</th>
            </tr>
          </thead>
          <tbody>
            {tableBody}
