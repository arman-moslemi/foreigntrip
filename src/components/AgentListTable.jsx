...
const AgentListTable = () =>{
  ...
  const[showEdit,setShowEdit] = React.useState(false);
    ...
    <tr key={tableRow.id} className="border-b border-b-borderGray">
      ...
      <td style={{fontFamily:'shabnam'}}  className="py-4 text-sm text-center px-4  ">{tableRow.edit}</td>
      <td style={{fontFamily:'shabnam'}}  className="py-4 text-sm text-center px-4  ">{tableRow.delete}</td>
      </tr> 
    )
    return(
        <>
  ...
  <thead className="bg-darkGray h-11 rounded-t-2xl w-full whitespace-nowrap overflow-x-scroll" style={{borderRadius:'20px'}}> 
    <tr className="text-white  p-6 whitespace-nowrap overflow-x-scroll" style={{borderRadius:'20px'}}>
      ...
      <th style={{fontFamily:'Shabnam'}} className="text-center   md:px-4" >ویرایش</th>
      <th style={{fontFamily:'Shabnam'}} className="text-center   md:px-4" >حذف</th>
    </tr>
  </thead>
  <tbody>
   {tableBody}
